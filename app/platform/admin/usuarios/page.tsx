...
export default async function UsuariosPage({
  ...
  let query = supabase
    .from("profiles")
    .select("id, nombre, apellido, email, dni, celular, role, socio, socio_desde, cuota_mensual, created_at")
    .order("apellido");

  if (q.trim()) {
    const needle = q.trim();
    query = query.or(`nombre.ilike.%${needle}%,apellido.ilike.%${needle}%,dni.ilike.%${needle}%,email.ilike.%${needle}%`);
  }
