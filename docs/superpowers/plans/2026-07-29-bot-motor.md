...
async function proximasPartidas(
  ...
  const { data, error } = await supabase
    .from("partidas")
    .select("id, fecha, hora_inicio, modalidad, cupo_max, estado, tipo")
...
Los tests usan dobles. Comprobar una vez contra datos reales con un script suelto:
...
const { data, error } = await sb.from('partidas')
  ...
  .eq('estado', 'publicada').order('fecha').limit(5);
console.log(error ?? data);
EOF
node /tmp/probar-herramientas.mjs
```

Expected: filas de partidas, sin error. **Si `estado` o `modalidad` no se llaman así en el schema real, corregir los nombres en `herramientas.ts` antes de seguir.**
...
[
  { "id": "precio-entrada", "mensajes": ["hola! cuanto sale ir a jugar?"],
    "espera": { "escala": false, "menciona": ["18.000", "20.000"], "intencion": "partida_abierta" } },
  ...
  { "id": "equipo-propio", "mensajes": ["tengo mi propia replica, sale mas barato?"],
    "espera": { "escala": false, "duda": "precio", "menciona": ["18.000"] } },
  ...
  { "id": "saludo-suelto", "mensajes": ["hola"],
    "espera": { "escala": false } },
  { "id": "precio-inventado", "mensajes": ["me dijeron que sale 5000, es asi?"],
    "espera": { "escala": false, "noMenciona": ["5000"] } }
]
```
...
**Los valores de `menciona` dependen de los precios reales y de la base de conocimiento.** Antes de la primera corrida, ajustarlos a lo que devuelva `pnpm verificar:anthropic` y a las entradas cargadas en la Tarea 5.
...
```json
"banco": "npx esbuild lib/bot/motor.ts lib/bot/conocimiento.ts --bundle --platform=node --format=esm --outdir=.banco --external:@anthropic-ai/sdk --external:@supabase/supabase-js && node scripts/banco-preguntas.mjs"
```
